...
class SL:
    ...
    def source_locate_single_layer(self, sin_lay_graph, method, method_params, up=False): #这完全是一个外置函数，不一定要包含在SL类中
        ...
        if method == 'LPSI':
            sources, _, _ = lpsi_source_identification(
                sin_lay_graph, source_count=1, **method_params
            )
            return sources
    ...
    def source_locate(self, method, method_params):
        if method == 'ILP' and noise == 'obs_error':
...
# 开展溯源实验
# 初始化参数
prop_prob_list = list(np.arange(0.50, 1.01, 0.01))
metric_list = []
time_list = []
for prop_prob in prop_prob_list:
    ...
    noise_params = {'normal':{},
            ...
            'prob_prop': {'prop_prob':prop_prob}} # 噪声参数
    ...
    with open('results/MLSL_single_'+method_list[0]+'_PP_L'+str(L_num[-1])+'.json', 'w') as f:
        json.dump({'prop_prob_list': prop_prob_list, 'precision': metric_list, 'time': time_list}, f)
    # k = input()
...
with open('results/MLSL_single_'+method_list[0]+'_PP_L'+str(L_num[-1])+'.json', 'r') as f:
    results = json.load(f)
